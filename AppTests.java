  ...
  public void run() 
  {
    ...
    switch( app_number ) 
    {
      ...
      case 2:
         ...
         msg_q = MakeSystemCall(SOSSyscallIntHandler.CreateMessageQueueSystemCall, 0, 0);
         SIM.Trace(SIM.TraceApp, "Message queue number " + msg_q);
			SIM.Trace(SIM.TraceApp, "Starting message receiver app");
         MakeSystemCall(SOSSyscallIntHandler.CreateProcessSystemCall, 3, msg_q);
         ...
         {
            ++counter;
	         //Build message, one word only
	         //Message buffer at cells 200-207
	         SIM.hw.SetCell(200, new Integer(counter));
	         //Send message system call
	         ret = MakeSystemCall(SOSSyscallIntHandler.SendMessageSystemCall, 200, msg_q);
            ...
            else
            {
		      ...
		      //Delay so all message buffers are not used
      ...
      case 3:
      msg_q = arg;
      SIM.Trace(SIM.TraceApp, "Starting message receiving loop, queue " + msg_q);
      
      while(true)
      {
	      ret = MakeSystemCall(SOSSyscallIntHandler.ReceiveMessageSystemCall, 200, msg_q);
         
         if(ret == 0)
          SIM.Trace(SIM.TraceApp, "RECEIVED message " + SIM.hw.GetCellAsInt(200));
      ...
      case 4:
         ...
         while(counter < 6) 
         {
            ...
            ret = MakeSystemCall(SOSSyscallIntHandler.DiskWriteSystemCall, 300, 0);
            
            if(ret != 0) 
               break;
               
	         SIM.Trace(SIM.TraceApp, "Disk write ret=" + ret);
	         ret = MakeSystemCall(SOSSyscallIntHandler.DiskReadSystemCall, 400, 0);
        
            if(ret != 0) 
               break;
	
            SIM.Trace(SIM.TraceApp, "Disk write ret=" + ret);
	         SIM.Trace(SIM.TraceApp, "wrote " + counter + ", read=" + SIM.hw.GetCellAsInt(400));
	         ++counter;
         }
