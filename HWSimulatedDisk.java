...
3/17 HWSimulatedDisk.java
Simulated Hardware - Disk
Simple Disk Simulation, contains it's own thread delayed at 500 ms.
The disk thread loop looks for disk requests and handles them.
*/
...
class HWSimulatedDisk implements Runnable
{
  ...
  {//Allocate space
   disk_data = new int[SOSDiskDriver.SizeOfDiskPage*SOSDiskDriver.NumberOfDiskPages];
  }
  ...
  {//Wait until the disk is not busy
    ...
    //Delay for the seek, latency and transfer. Delays at 500 ms
    ...
    //Begin the simulation. Move data between the simulated disk and the simulated memory
    for( int i = 0; i < SOSDiskDriver.SizeOfDiskPage; ++i )
    {
      if( is_read )
      {
	      SIM.hw.SetCellUnmapped(memory_address++, disk_data[disk_offset++]);
      } 
      
      else
      {
	      disk_data[disk_offset++] = SIM.hw.GetCellUnmappedAsInt(memory_address++);
      }
    }

    //Determine if signal or interrupt
    SIM.Trace(SIM.TraceHWDisk, "control_reg=" + Integer.toBinaryString(control_reg));
    
    if((control_reg & (1<<30)) != 0)
    {
      SIM.Trace(SIM.TraceHWDisk, "Signal disk interrupt");
      //signal a disk interrupt
      SIMIntHandler handler = (SIMIntHandler)SIM.hw.GetCellUnmapped(SIM.DiskIntIntVector);
...
It waits for timer requests and counts them down 100 ms.
*/
  ...
  public synchronized void tick(int increment)
  {
    if(time_left > 0)
    {
      ...
      if(time_left <= 0) 
      {
	      ...
	      SIMIntHandler handler = (SIMIntHandler)SIM.hw.GetCellUnmapped(SIM.TimerIntVector);
	      handler.HandleInterrupt(0);
...
{//HW Simulation class
  ...
  public Object GetCell(int addr)
  {
    ...
    else
    {
      ...
      SIMIntHandler handler = (SIMIntHandler)SIM.hw.GetCellUnmapped(SIM.ProgErrIntIntVector);
  ...
  public void SystemCall(int addr)
  {
    ...
    SIM.Trace(SIM.TraceHWSim, "HW system call complete: Resuming pid " + cur_proc);
    return;
  }
  ...
  public void RunProcess(int pid)
  {
    ...
    SIM.Trace(SIM.TraceHWSim, "Resuming user thread " + ProcessThread[pid].toString());
  ...
  {
    SIM.TheSIM.sosPauser.checkIfPaused();
    SIM.Trace(SIM.TraceHWSim, "No process to run.  Suspending " + Thread.currentThread().toString());
    
    if(theDisk.GetStatusRegister() == 0 && hwTimer.GetTimeLeft() == 0)
    {
      SIM.Trace(SIM.TraceAlways, "System idle. No ready processes, no timer, no disk IO");
    }
  ...
  public void CreateProcess(int app_number, int arg, int pid)
  {
    ...
    switch(app_number) 
    {
      case 0:
         ...
         SIM.Trace(SIM.TraceHWSim, "create and suspend user 0 " + t.toString());
      ...
      default:
         ...
         SIM.TheSIM.threadList.addElement(t);
         SIM.Trace(SIM.TraceHWSim,"create and suspend user " + app_number + " " + t.toString());
         t.setPriority(Thread.NORM_PRIORITY);
         t.start();
         t.suspend();
         break;
    ...
    SIM.TheSIM.proc_tabs.addTab("Proc"+pid, null, pscroll, "Trace for process "+pid);
  }
}
